...
from module_sample_sql_alchemy.sql_alchemy_objects.db1_main.entities.template_data_type_mapping_test import \
    Db1TemplateDataTypeMappingTest
import \
    module_sample_sql_alchemy.sql_alchemy_objects.db1_main.value_objects.template_data_type_mapping_test_vo as value_objects
...
@sql_alchemy_func
async def save(db: AsyncSession, entity: Db1TemplateDataTypeMappingTest) -> Db1TemplateDataTypeMappingTest:
...
async def delete_all(db: AsyncSession):
    stmt = select(Db1TemplateDataTypeMappingTest)
...
async def delete_by_id(db: AsyncSession, pk: int):
    stmt = select(Db1TemplateDataTypeMappingTest).where(Db1TemplateDataTypeMappingTest.uid == pk)
    result = await db.execute(stmt)
...
# 모든 데이터 검색
@sql_alchemy_func
async def find_all(db: AsyncSession) -> Sequence[Db1TemplateDataTypeMappingTest]:
    stmt = select(Db1TemplateDataTypeMappingTest)
    result = await db.execute(stmt)
    entity_list = result.scalars().all()
    return entity_list
...
@sql_alchemy_func
async def find_by_id(db: AsyncSession, pk: int) -> Optional[Db1TemplateDataTypeMappingTest]:
    stmt = select(Db1TemplateDataTypeMappingTest).where(Db1TemplateDataTypeMappingTest.uid == pk)
    result = await db.execute(stmt)
    entity = result.scalar_one_or_none()
    return entity
